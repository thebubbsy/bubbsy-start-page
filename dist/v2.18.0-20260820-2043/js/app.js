...
(function () {
  ...
  function renderWorldClocks() {
    const clocks = (appData.world_clocks && appData.world_clocks.length >= 8) ? appData.world_clocks : DEFAULT_WORLD_CLOCKS;
    elWorldClocks.innerHTML = clocks.map(c => {
    ...
    elWorldClocks.querySelectorAll('.clock-item').forEach(item => {
      item.addEventListener('click', () => {
        ...
        showToast(`Copied timestamp: ${copyText}`);
      });
    });
  ...
  function createWidgetCard(w) {
    ...
    (w.links || []).forEach(link => {
      ...
      li.addEventListener('contextmenu', (e) => {
        ...
        showToast(`[COPIED URL] ${link.title}`);
      });
  ...
  function renderPaletteResults(query) {
    ...
    const bangShortcuts = [
      { bang: '!abn', name: 'ABN Lookup & Corporate Lineage', icon: 'ABN', action: () => { elModalPalette.classList.remove('active'); openCorpRecon(); } },
      { bang: '!es', name: 'Voidtools Everything Local File Search', icon: 'ES', action: () => { elModalPalette.classList.remove('active'); setSearchMode('everything'); elMainSearch.focus(); } },
      { bang: '!trove', name: 'National Library of Australia Trove Archives', icon: 'TROVE', action: () => { elModalPalette.classList.remove('active'); setSearchMode('trove'); elMainSearch.focus(); } },
      { bang: '!shodan', name: 'Shodan IoT & IP Threat Surface', icon: 'SHODAN', action: () => { elModalPalette.classList.remove('active'); setSearchMode('shodan'); elMainSearch.focus(); } },
      { bang: '!vt', name: 'VirusTotal Threat Intelligence', icon: 'VT', action: () => { elModalPalette.classList.remove('active'); setSearchMode('virustotal'); elMainSearch.focus(); } },
      { bang: '!dork', name: 'Threat Dork & Surface Generator', icon: 'DORK', action: () => { elModalPalette.classList.remove('active'); openDorkGenerator(); } },
      { bang: '!geo', name: 'Australian Cadastre & Geo Recon', icon: 'GEO', action: () => { elModalPalette.classList.remove('active'); openGeoRecon(); } },
      { bang: '!corp', name: 'ASIC / ABR Corporate Registers', icon: 'CORP', action: () => { elModalPalette.classList.remove('active'); openCorpRecon(); } },
      { bang: '!defang', name: 'Cyber Defang / Refang IOC Normalizer', icon: 'DEFANG', action: () => { elModalPalette.classList.remove('active'); openDefanger(); } },
      { bang: '!radar', name: 'Threat Radar & ACSC Essential 8', icon: 'RADAR', action: () => { elModalPalette.classList.remove('active'); openThreatRadar(); } },
      { bang: '!graph', name: 'Visual Investigation Link Graph', icon: 'GRAPH', action: () => { elModalPalette.classList.remove('active'); openInvestigationGraph(); } }
    ];
  ...
  const elHarvestPoolNames = document.getElementById('harvest-pool-names');
  const elHarvestPoolLocations = document.getElementById('harvest-pool-locations');
  const elHarvestPoolOrgs = document.getElementById('harvest-pool-orgs');
  ...
  const TRIAGE_PLATFORMS = [
    { id: 'github', name: 'GitHub', category: 'dev', desc: 'Code Repositories & Open Source' },
    { id: 'reddit', name: 'Reddit', category: 'social', desc: 'Discussions, Karma & Subreddits' },
    { id: 'keybase', name: 'Keybase', category: 'social', desc: 'PGP Keys & Verified Social Proofs' },
    { id: 'bluesky', name: 'Bluesky', category: 'social', desc: 'AT Protocol Decentralized Social' },
    { id: 'gitlab', name: 'GitLab', category: 'dev', desc: 'GitLab Projects & DevOps' },
    { id: 'whirlpool', name: 'Whirlpool Forums', category: 'australian', desc: 'Australian Broadband & IT Forums' },
    { id: 'devto', name: 'Dev.to', category: 'dev', desc: 'Developer Community & Tech Articles' },
    { id: 'ozbargain', name: 'OzBargain AU', category: 'australian', desc: 'Australian Deals & Merchant Accounts' },
    { id: 'dockerhub', name: 'Docker Hub', category: 'dev', desc: 'Container Images & Registries' }
  ];
  ...
  function updateHarvestedPoolBar() {
    ...
    if (harvestedContext.confirmedNames.size > 0) {
      elHarvestPoolNames.style.display = 'inline-flex';
      elHarvestPoolNames.textContent = `👤 ${Array.from(harvestedContext.confirmedNames).slice(0, 2).join(', ')}`;
    } else {
      elHarvestPoolNames.style.display = 'none';
    }

    if (harvestedContext.confirmedLocations.size > 0) {
      elHarvestPoolLocations.style.display = 'inline-flex';
      elHarvestPoolLocations.textContent = `📍 ${Array.from(harvestedContext.confirmedLocations).slice(0, 2).join(', ')}`;
    } else {
      elHarvestPoolLocations.style.display = 'none';
    }

    if (harvestedContext.confirmedOrgs.size > 0) {
      elHarvestPoolOrgs.style.display = 'inline-flex';
      elHarvestPoolOrgs.textContent = `🏢 ${Array.from(harvestedContext.confirmedOrgs).slice(0, 2).join(', ')}`;
    } else {
      elHarvestPoolOrgs.style.display = 'none';
    }
  }
  ...
  async function startGuidedDisambiguation(query = '') {
    ...
    harvestedContext.confirmedHandles.add(q.toLowerCase());
    currentTriagePlatformIndex = 0;
  ...
  async function renderTriageActivePlatform() {
    ...
    if (elSocialTriageStepperText) {
      elSocialTriageStepperText.textContent = `PLATFORM ${currentTriagePlatformIndex + 1} OF ${TRIAGE_PLATFORMS.length}`;
    }
    if (elTriagePlatformName) {
      elTriagePlatformName.innerHTML = `${plat.name} <span class="brand-tag" style="margin-left:6px;">${plat.desc}</span>`;
    }

    // Build context string showing how newly harvested data is enriching this search
    let queryContextDesc = `Query: "${q}"`;
    ...
    if (names.length > 0) queryContextDesc += ` • Names: [${names.join(', ')}]`;
    if (locs.length > 0) queryContextDesc += ` • Locations: [${locs.join(', ')}]`;
    if (orgs.length > 0) queryContextDesc += ` • Orgs: [${orgs.join(', ')}]`;

    if (elTriageQueryContext) {
      elTriageQueryContext.textContent = queryContextDesc;
    }

    elSocialTriageCandidatesList.innerHTML = `
      <div style="padding:20px;text-align:center;color:var(--text-muted);font-family:var(--font-mono);font-size:0.75rem;">
        <svg class="spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-bottom:6px;"><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg>
        <div>Fetching Top 5 Candidates on ${plat.name} with harvested intelligence...</div>
      </div>
    `;
  ...
  async function generateTop5CandidatesForPlatform(plat, baseQuery, context) {
    ...
    const primaryName = names[0] || baseQuery;

    // 1. Direct candidate matching base query
    const exactHandle = baseQuery.replace(/\s+/g, '').toLowerCase();

    // Specific live fetching for GitHub
    if (plat.id === 'github') {
      try {
        const ghSearch = await fetch(`https://api.github.com/search/users?q=${encodeURIComponent(primaryName || baseQuery)}&per_page=5`);
        if (ghSearch.ok) {
          const ghData = await ghSearch.json();
          const items = ghData.items || [];
          for (const item of items.slice(0, 5)) {
            candidates.push({
              key: `github_${item.login}`,
              platform: 'GitHub',
              platform_id: 'github',
              handle: item.login,
              ...
              bio: `Public Repos & Open Source Profile (${item.type})`,
              location: context.confirmedLocations.size ? Array.from(context.confirmedLocations)[0] : '',
              company: context.confirmedOrgs.size ? Array.from(context.confirmedOrgs)[0] : '',
              stats: 'GitHub Developer Account'
            });
          }
        }
      } catch (e) {}
    ...
    // Specific live fetching for Bluesky
    if (plat.id === 'bluesky') {
      try {
        const bskySearch = await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.searchActors?q=${encodeURIComponent(primaryName || baseQuery)}&limit=5`);
        if (bskySearch.ok) {
          const bskyData = await bskySearch.json();
          const actors = bskyData.actors || [];
          for (const actor of actors.slice(0, 5)) {
            candidates.push({
              key: `bluesky_${actor.handle}`,
              platform: 'Bluesky',
              platform_id: 'bluesky',
              handle: actor.handle,
              display_name: actor.displayName || actor.handle,
              url: `https://bsky.app/profile/${actor.handle}`,
              avatar_url: actor.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(actor.displayName || actor.handle)}&background=0085ff&color=fff`,
              bio: actor.description || '',
              location: '',
              company: '',
              stats: 'AT Protocol Profile'
            });
          }
        }
      } catch (e) {}
    ...
    // If API returned fewer than 5 or other platform, synthesize intelligent query variations using harvested metadata
    const queryVariations = [
      exactHandle,
      primaryName !== baseQuery ? primaryName.replace(/\s+/g, '_').toLowerCase() : `${exactHandle}_sec`,
      primaryName !== baseQuery ? primaryName.replace(/\s+/g, '').toLowerCase() : `${exactHandle}_au`,
      `${exactHandle}_dev`,
      `${exactHandle}1`
    ];
    ...
    while (candidates.length < 5) {
      ...
      const vHandle = queryVariations[idx] || `${exactHandle}_${idx}`;
      const vName = primaryName || baseQuery;
      let pUrl = `https://${plat.id}.com/${vHandle}`;
      let pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=00f0ff&color=000`;

      if (plat.id === 'reddit') {
        pUrl = `https://www.reddit.com/user/${vHandle}`;
        pAvatar = `https://www.redditstatic.com/avatars/avatar_default_${idx}.png`;
        ...
        pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=1070e0&color=fff`;
      } else if (plat.id === 'ozbargain') {
        pUrl = `https://www.ozbargain.com.au/user/${vHandle}`;
        pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=10b981&color=fff`;
      } else if (plat.id === 'devto') {
        pUrl = `https://dev.to/${vHandle}`;
        pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=0a0a0a&color=fff`;
      } else if (plat.id === 'dockerhub') {
        pUrl = `https://hub.docker.com/u/${vHandle}`;
        pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=0db7ed&color=fff`;
      } else if (plat.id === 'gitlab') {
        pUrl = `https://gitlab.com/${vHandle}`;
        pAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(vName)}&background=fc6d26&color=fff`;
      }
      ...
      candidates.push({
        ...
        platform_id: plat.id,
        handle: vHandle,
        display_name: idx === 0 ? (primaryName || vHandle) : `${vName} (${vHandle})`,
        url: pUrl,
        avatar_url: pAvatar,
        bio: idx === 0 ? `Target subject candidate on ${plat.name}` : `Alias variant candidate for ${baseQuery}`,
        location: Array.from(context.confirmedLocations)[0] || '',
        company: Array.from(context.confirmedOrgs)[0] || '',
        stats: `${plat.desc}`
      });
    }
    ...
    if (!candidates || candidates.length === 0) {
      elSocialTriageCandidatesList.innerHTML = `
        <div style="padding:20px;text-align:center;color:var(--text-muted);">
          No candidate profiles discovered on ${plat.name}. Click Next Platform to continue.
        </div>
      `;
      return;
    }
      ...
      return `
        <div class="triage-candidate-card ${cardStateClass}" id="card-${cand.key}" data-key="${cand.key}">
          <div class="triage-candidate-left">
            <img class="triage-avatar" src="${cand.avatar_url}" alt="${escapeHtml(cand.display_name)}" onclick="window.open('${cand.url}', '_blank');" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(cand.handle)}&background=1e293b&color=fff';" title="Click to view profile on ${cand.platform}" />
            <div class="triage-details">
              ...
              ${cand.bio ? `<div class="triage-bio">${escapeHtml(cand.bio)}</div>` : ''}
          ...
          <div class="triage-decision-group">
            <button type="button" class="btn-decision btn-decision-yes ${decision === 'yes' ? 'active' : ''}" data-key="${cand.key}" data-action="yes">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
              YES (Match)
            </button>
            <button type="button" class="btn-decision btn-decision-unsure ${decision === 'unsure' ? 'active' : ''}" data-key="${cand.key}" data-action="unsure">
            ...
            <button type="button" class="btn-decision btn-decision-no ${decision === 'no' ? 'active' : ''}" data-key="${cand.key}" data-action="no">
  ...
  function handleTriageDecision(candidate, action, plat) {
    ...
    if (action === 'yes') {
      // 1. Harvest metadata points from this confirmed profile
      if (candidate.display_name && candidate.display_name.toLowerCase() !== candidate.handle.toLowerCase()) {
        harvestedContext.confirmedNames.add(candidate.display_name);
      }
      harvestedContext.confirmedHandles.add(candidate.handle);
      if (candidate.location) harvestedContext.confirmedLocations.add(candidate.location);
      if (candidate.company) harvestedContext.confirmedOrgs.add(candidate.company);

      // Add to confirmed profile list if not already present
      if (!harvestedContext.confirmedProfiles.some(p => p.url === candidate.url)) {
        harvestedContext.confirmedProfiles.push(candidate);
      }

      updateHarvestedPoolBar();
      showToast(`Confirmed ${plat.name} profile! Harvested metadata & updated cross-platform query context.`);

      // Re-render this card to show emerald glow
      const cardEl = document.getElementById(`card-${candidate.key}`);
      if (cardEl) {
        cardEl.className = 'triage-candidate-card decision-yes';
      }

      // Automatically advance to next platform after brief delay to accelerate workflow
      setTimeout(() => {
        if (currentTriagePlatformIndex < TRIAGE_PLATFORMS.length - 1) {
          currentTriagePlatformIndex++;
          renderTriageActivePlatform();
        } else {
          showToast('Disambiguation complete across all platforms! View the Consolidated Dossier.');
        }
      }, 550);

    } else if (action === 'unsure') {
      const cardEl = document.getElementById(`card-${candidate.key}`);
      if (cardEl) cardEl.className = 'triage-candidate-card decision-unsure';
      showToast('Marked candidate as Unsure');
    } else if (action === 'no') {
      const cardEl = document.getElementById(`card-${candidate.key}`);
      if (cardEl) cardEl.className = 'triage-candidate-card decision-no';
      // Remove from confirmed if previously set
      harvestedContext.confirmedProfiles = harvestedContext.confirmedProfiles.filter(p => p.url !== candidate.url);
      updateHarvestedPoolBar();
  ...
  elBtnClearHarvestedIntel?.addEventListener('click', () => {
    ...
    renderTriageActivePlatform();
    showToast('Cleared harvested intelligence context');
  });
  ...
  function generateFallbackProfiles(u) {
    return [
      { platform: 'GitHub', platform_id: 'github', category: 'dev', url: `https://github.com/${u}`, avatar_url: `https://avatars.githubusercontent.com/${u}`, display_name: u, bio: '', stats: 'Code Repositories', found: false },
      { platform: 'Reddit', platform_id: 'reddit', category: 'social', url: `https://www.reddit.com/user/${u}`, avatar_url: `https://www.redditstatic.com/avatars/avatar_default_0.png`, display_name: u, bio: '', stats: 'Discussions & Posts', found: false },
      { platform: 'Keybase', platform_id: 'keybase', category: 'social', url: `https://keybase.io/${u}`, avatar_url: `https://keybase.io/${u}/picture`, display_name: u, bio: '', stats: 'Cryptographic Identity', found: false },
      { platform: 'Whirlpool Forums', platform_id: 'whirlpool', category: 'australian', url: `https://forums.whirlpool.net.au/user/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=1070e0&color=fff`, display_name: u, bio: '', stats: 'Australian IT & Broadband', found: false },
      { platform: 'Overclockers AU', platform_id: 'ocau', category: 'australian', url: `https://forums.overclockers.com.au/members/?username=${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=f59e0b&color=000`, display_name: u, bio: '', stats: 'Australian Hardware Community', found: false },
      { platform: 'OzBargain AU', platform_id: 'ozbargain', category: 'australian', url: `https://www.ozbargain.com.au/user/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=10b981&color=fff`, display_name: u, bio: '', stats: 'Australian Deals Community', found: false },
      { platform: 'X / Twitter', platform_id: 'twitter', category: 'social', url: `https://x.com/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=000000&color=fff`, display_name: u, bio: '', stats: 'Microblogging Profile', found: false },
      { platform: 'Bluesky', platform_id: 'bluesky', category: 'social', url: `https://bsky.app/profile/${u}.bsky.social`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=0085ff&color=fff`, display_name: u, bio: '', stats: 'Decentralized Social', found: false },
      { platform: 'Telegram', platform_id: 'telegram', category: 'social', url: `https://t.me/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=229ed9&color=fff`, display_name: u, bio: '', stats: 'Encrypted Messaging', found: false },
      { platform: 'LinkedIn', platform_id: 'linkedin', category: 'social', url: `https://www.linkedin.com/in/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=0077b5&color=fff`, display_name: u, bio: '', stats: 'Professional Identity', found: false },
      { platform: 'GitLab', platform_id: 'gitlab', category: 'dev', url: `https://gitlab.com/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=fc6d26&color=fff`, display_name: u, bio: '', stats: 'GitLab Projects', found: false },
      { platform: 'PyPI', platform_id: 'pypi', category: 'dev', url: `https://pypi.org/user/${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=3775a9&color=ffd343`, display_name: u, bio: '', stats: 'Python Package Index', found: false },
      { platform: 'npm', platform_id: 'npm', category: 'dev', url: `https://www.npmjs.com/~${u}`, avatar_url: `https://ui-avatars.com/api/?name=${encodeURIComponent(u)}&background=cb3837&color=fff`, display_name: u, bio: '', stats: 'npm Package Author', found: false }
    ];
  }
  ...
  function renderSocialReconProfiles() {
    const u = elSocialInput.value.trim();
    if (!u && currentScannedProfiles.length === 0) {
      elSocialResults.innerHTML = `
        <div class="es-empty-state" style="grid-column:1/-1;">
          Enter any username, handle, or alias to scan and consolidate identity profiles across 30+ intelligence platforms.
        </div>
      `;
      ...
      return `
        <div class="social-profile-card ${isConfirmed || isVerified ? 'verified-found' : ''}" data-url="${p.url}">
          <div class="social-card-top">
            <div class="social-avatar-wrapper" title="Click to view avatar" onclick="window.open('${avatarSrc}', '_blank');">
          ...
          <div class="social-card-actions">
            <label class="social-consolidate-checkbox" title="Include in consolidated persona">
              <input type="checkbox" class="chk-consolidate-profile" data-url="${p.url}" ${isConfirmed ? 'checked' : ''} />
              <span>Confirmed</span>
            </label>
            <div style="display:flex;gap:6px;">
              <button type="button" class="btn-icon" style="padding:1px 6px;font-size:0.64rem;" onclick="addNodeToGraph('${escapeRegex(p.display_name || u)} (${escapeRegex(p.platform)})', 'person'); showToast('Added profile to Link Graph!');">
                + Graph
              </button>
  ...
  document.getElementById('btn-social-consolidate-person')?.addEventListener('click', () => {
    ...
    const locations = Array.from(harvestedContext.confirmedLocations).join(' • ');
    const orgs = Array.from(harvestedContext.confirmedOrgs).join(', ');

    let dossierMd = `# 🕵️ CONSOLIDATED PERSONA DOSSIER: ${primaryName} (@${u})\n\n`;
    dossierMd += `**Primary Target**: \`${primaryName}\` (@${u})\n`;
    if (names.length > 0) dossierMd += `**Harvested Real Names**: ${names.join(', ')}\n`;
    if (locations) dossierMd += `**Discovered Locations**: ${locations}\n`;
    if (orgs) dossierMd += `**Discovered Organizations**: ${orgs}\n`;
    dossierMd += `**Confirmed Accounts (${confirmed.length})**:\n\n`;

    if (confirmed.length === 0) {
      dossierMd += `_No accounts explicitly confirmed via YES yet._\n`;
    } else {
      confirmed.forEach(p => {
        dossierMd += `### [${p.platform}] ${p.display_name} (@${p.handle || u})\n`;
        dossierMd += `- **URL**: ${p.url}\n`;
        if (p.bio) dossierMd += `- **Bio**: ${p.bio}\n`;
        if (p.location) dossierMd += `- **Location**: ${p.location}\n`;
        if (p.company) dossierMd += `- **Org/Company**: ${p.company}\n`;
        dossierMd += `\n`;
      });
    }

    navigator.clipboard.writeText(dossierMd);
    showToast(`Copied Consolidated Persona Dossier for "${primaryName}" to clipboard!`);
  });
  ...
  document.getElementById('btn-social-add-to-graph')?.addEventListener('click', () => {
    ...
    const rootNodeId = addNodeToGraph(`Person: ${primaryName} (@${u})`, 'person');

    confirmed.forEach(p => {
      const platNodeId = addNodeToGraph(`${p.platform}: ${p.display_name || p.handle || u}`, 'social');
      if (typeof addEdgeToGraph === 'function') {
        addEdgeToGraph(rootNodeId, platNodeId);
      }
    });

    harvestedContext.confirmedLocations.forEach(loc => {
      const locNodeId = addNodeToGraph(`Location: ${loc}`, 'geo');
      if (typeof addEdgeToGraph === 'function') {
        addEdgeToGraph(rootNodeId, locNodeId);
      }
    });

    openInvestigationGraph();
    elModalSocialRecon.classList.remove('active');
    showToast(`Sent Persona "${primaryName}", ${confirmed.length} accounts & locations to Link Graph!`);
  });
  ...
  function renderCorpRecon(query = '') {
    ...
    if (elCorpValidationBadge) {
      if (!q) {
        ...
        elCorpValidationBadge.title = isValid 
          ? `ASIC Statutory Checksum: Complement of weighted sum modulo 10 matches check digit [VALID]`
    ...
    if (btnCopyAbn) {
      if (digitsOnly.length === 11 || digitsOnly.length === 9) {
        ...
        btnCopyAbn.onclick = () => {
          ...
          showToast(`Copied formatted ${isAbn ? 'ABN' : 'ACN'}: ${formatted}`);
        };
      } else {
...
<head>
  ...
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #030712; color: #f9fafb; padding: 32px; line-height: 1.6; max-width: 900px; margin: 0 auto; }
    h1 { color: #00f0ff; border-bottom: 2px solid #00f0ff; padding-bottom: 8px; }
    h2 { color: #10b981; margin-top: 28px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 4px; }
    ...
    code { font-family: monospace; background: rgba(255,255,255,0.1); padding: 2px 6px; border-radius: 3px; }
    a { color: #00f0ff; text-decoration: none; }
    a:hover { text-decoration: underline; }
    .badge { display: inline-block; padding: 2px 8px; border-radius: 4px; background: rgba(16,185,129,0.2); color: #10b981; font-size: 0.8rem; }
  </style>
</head>
<body>
  <h1>${escapeHtml(caseName)}</h1>
  <p><span class="badge">BUBBSY OSINT DOSSIER</span> <strong>Generated:</strong> ${new Date().toISOString()}</p>
  ...
  document.getElementById('btn-export-csv-catalog')?.addEventListener('click', () => {
    ...
    a.click();
    showToast('Exported 1,699 OSINT Tools Catalog (CSV)!');
  });
  ...
  document.getElementById('btn-dork-copy-raw-urls')?.addEventListener('click', () => {
    ...
    showToast(`Copied ${urls.length} Raw Dork URLs!`);
  });
  ...
  document.getElementById('btn-geo-copy-geohash')?.addEventListener('click', () => {
    ...
    showToast(`Geohash copied: ${geohash}`);
  });
  ...
  document.getElementById('btn-geo-copy-mgrs')?.addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(cadStr);
    showToast(`Copied CAD Dispatch Telemetry: ${cadStr}`);
  });
  ...
  document.getElementById('btn-geo-copy-solar')?.addEventListener('click', () => {
    ...
    navigator.clipboard.writeText(text);
    showToast(`Copied Solar Shadow Intel: ${text}`);
  });
  ...
  document.querySelectorAll('.bang-cheatsheet-item').forEach(item => {
    item.addEventListener('click', () => {
      ...
      elModalShortcuts.classList.remove('active');
      elMainSearch.value = bang + ' ';
      elMainSearch.focus();
      handleSearchInput(elMainSearch.value);
