...
(function () {
  ...
  // =========================================================================
  // 3B. SOCIAL HANDLE & USERNAME RECON ENGINE (SHERLOCK / MAIGRET / LIVE PROBE)
  // =========================================================================
  ...
  let activeSocialCategoryFilter = 'all'; // 'all', 'verified', 'australian', 'dev', 'social'
  let activeSocialKeywordFilter = null; // clicked keyword string or null
  let currentScannedProfiles = [];
  let currentAggregatedKeywords = [];
  let currentDiscoveredNames = [];
  let selectedProfileUrls = new Set();
  let isSocialScanning = false;

  function openSocialRecon(username = '') {
    elModalSocialRecon.classList.add('active');
    if (username) {
      elSocialInput.value = username;
      triggerSocialScan(username);
    } else {
      if (currentScannedProfiles.length === 0 && elSocialInput.value.trim()) {
        triggerSocialScan(elSocialInput.value.trim());
      } else {
  ...
  async function triggerSocialScan(username = '') {
    const u = (username || elSocialInput.value).trim();
    if (!u) {
      showToast('Please enter a username to scan');
      return;
    }

    isSocialScanning = true;
    elBtnSocialSearch.disabled = true;
    elBtnSocialSearch.innerHTML = `
    ...
    elSocialResults.innerHTML = `
      <div class="es-empty-state" style="grid-column:1/-1;padding:30px 10px;">
        <div style="font-size:1.05rem;font-weight:700;color:var(--accent-cyan);margin-bottom:8px;">
          Probing Cross-Platform Registries for "${escapeHtml(u)}"...
        </div>
        <div style="font-size:0.75rem;color:var(--text-muted);font-family:var(--font-mono);">
          Querying GitHub, Reddit, Keybase, GitLab, Bluesky, Dev.to, Gravatar, Australian Forums & Social Networks...
        </div>
      </div>
    `;
    ...
    try {
      ...
      currentAggregatedKeywords = [];
      currentDiscoveredNames = [];
    } finally {
      isSocialScanning = false;
      elBtnSocialSearch.disabled = false;
      elBtnSocialSearch.innerHTML = `
      ...
      currentScannedProfiles.forEach(p => {
        if (p.found) selectedProfileUrls.add(p.url);
      });
  ...
  function renderSocialReconProfiles() {
    ...
    // Render visual identity cards
    elSocialResults.innerHTML = displayProfiles.map(p => {
      const isChecked = selectedProfileUrls.has(p.url);
      ...
      return `
        <div class="social-profile-card ${isVerified ? 'verified-found' : ''}" data-url="${p.url}">
          <div class="social-card-top">
            ...
            <div class="social-card-identity">
              ...
              <div class="social-card-platform">
                ...
                ${isVerified ? '<span style="color:var(--accent-green);font-weight:700;">• VERIFIED</span>' : ''}
              </div>
            </div>
          </div>
          ...
          <div class="social-card-actions">
            <label class="social-consolidate-checkbox" title="Include in consolidated identity">
              <input type="checkbox" class="chk-consolidate-profile" data-url="${p.url}" ${isChecked ? 'checked' : ''} />
    ...
    elSocialResults.querySelectorAll('.chk-consolidate-profile').forEach(chk => {
      chk.addEventListener('change', (e) => {
        ...
        if (e.target.checked) {
          selectedProfileUrls.add(url);
        } else {
          selectedProfileUrls.delete(url);
        }
      });
    });
  }
  ...
  document.getElementById('btn-social-recon-search').addEventListener('click', () => triggerSocialScan(elSocialInput.value.trim()));
  document.getElementById('social-recon-input').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') triggerSocialScan(elSocialInput.value.trim());
  });

  document.getElementById('btn-social-select-all')?.addEventListener('click', () => {
    currentScannedProfiles.forEach(p => selectedProfileUrls.add(p.url));
    renderSocialReconProfiles();
    showToast(`Selected all ${selectedProfileUrls.size} profiles`);
  });

  document.getElementById('btn-social-deselect-all')?.addEventListener('click', () => {
    selectedProfileUrls.clear();
    renderSocialReconProfiles();
    showToast('Deselected all profiles');
  });

  document.getElementById('btn-social-consolidate-person')?.addEventListener('click', () => {
    const u = elSocialInput.value.trim() || 'target_user';
    const selected = currentScannedProfiles.filter(p => selectedProfileUrls.has(p.url));
    if (selected.length === 0) {
      showToast('Check at least one profile to consolidate');
      return;
    }

    const primaryName = currentDiscoveredNames[0] || u;
    const platformsList = selected.map(p => p.platform).join(', ');
    const locations = Array.from(new Set(selected.map(p => p.location).filter(Boolean))).join(' • ');

    let dossierMd = `# 🕵️ CONSOLIDATED IDENTITY DOSSIER: ${primaryName} (@${u})\n\n`;
    dossierMd += `**Target Handle**: \`@${u}\`\n`;
    dossierMd += `**Discovered Names**: ${currentDiscoveredNames.join(', ') || 'N/A'}\n`;
    if (locations) dossierMd += `**Locations**: ${locations}\n`;
    dossierMd += `**Consolidated Platforms (${selected.length})**: ${platformsList}\n\n`;
    dossierMd += `## Verified Profile Accounts\n\n`;
    selected.forEach(p => {
      dossierMd += `### [${p.platform}] ${p.display_name}\n`;
      dossierMd += `- **URL**: ${p.url}\n`;
      if (p.bio) dossierMd += `- **Bio**: ${p.bio}\n`;
      if (p.location) dossierMd += `- **Location**: ${p.location}\n`;
      if (p.company) dossierMd += `- **Org/Company**: ${p.company}\n`;
      if (p.stats) dossierMd += `- **Stats**: ${p.stats}\n`;
      dossierMd += `\n`;
    });

    navigator.clipboard.writeText(dossierMd);
    showToast(`Consolidated ${selected.length} profiles for "${primaryName}" copied as Markdown Dossier!`);
  });
  ...
  document.getElementById('btn-social-add-to-graph').addEventListener('click', () => {
    ...
    if (selected.length > 0) {
      selected.forEach(p => {
        const platNodeId = addNodeToGraph(`${p.platform}: ${p.display_name || u}`, 'social');
        // Add edge from person to platform node if investigation graph supports addEdge
    ...
    showToast(`Sent Person "${primaryName}" & ${selected.length} platform accounts to Link Graph!`);
  });

  document.getElementById('btn-social-launch-all').addEventListener('click', () => {
    const selected = currentScannedProfiles.filter(p => selectedProfileUrls.has(p.url));
    if (!selected.length) {
      showToast('Select at least one profile to launch');
      return;
    }
    selected.forEach(p => {
      if (p.url) window.open(p.url, '_blank', 'noopener,noreferrer');
    });
    showToast(`Launched ${selected.length} profile tabs!`);
  });
