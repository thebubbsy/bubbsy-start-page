...
(function () {
  ...
  function renderRadarItems(filter = '') {
    ...
    if (!filtered.length) {
      elRadarItemsList.innerHTML = `<div class="es-empty-state">No advisories match filter "${escapeHtml(filter)}"</div>`;
      return;
    }
  ...
  function renderCorpRecon(query = '') {
    ...
    const cleanedDigits = q.replace(/\s+/g, '');
  ...
  function extractAllIOCs(text) {
    ...
    // Filter out matches that are part of emails or purely IP numbers
  ...
  document.getElementById('btn-defang-copy').addEventListener('click', () => {
