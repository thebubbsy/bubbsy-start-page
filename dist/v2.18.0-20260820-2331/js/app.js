...
(function () {
  ...
  function renderPaletteResults(query) {
    ...
    const actions = [
      ...
      { id: 'act_typography', title: 'Customize Typography, Font Size, Thickness & High-Contrast Colors', category: 'ACTIONS', icon: 'TYPO', action: () => { elModalPalette.classList.remove('active'); openTypographyModal(); } }
    ];

    const bangShortcuts = [
      { bang: '!aistudio', name: 'Google AI Studio Developer Prompt Studio', icon: 'AI', action: () => { elModalPalette.classList.remove('active'); setSearchMode('aistudio'); elMainSearch.focus(); } },
      { bang: '!gemini', name: 'Google Gemini Multimodal Assistant', icon: 'GEMINI', action: () => { elModalPalette.classList.remove('active'); setSearchMode('aistudio'); elMainSearch.focus(); } },
      { bang: '!typo', name: 'Typography, Font Size & High-Contrast Colors', icon: 'TYPO', action: () => { elModalPalette.classList.remove('active'); openTypographyModal(); } },
      { bang: '!font', name: 'Font Scaling & Text Legibility Suite', icon: 'FONT', action: () => { elModalPalette.classList.remove('active'); openTypographyModal(); } },
  ...
  async function renderTriageActivePlatform() {
    ...
    if (elTriagePlatformName) {
      elTriagePlatformName.innerHTML = `${plat.icon} ${plat.name} <span class="brand-tag" style="margin-left:6px;">${plat.desc}</span>`;
    }
      ...
      return `
        <div class="triage-candidate-card ${cardStateClass}" id="card-${cand.key}" data-key="${cand.key}">
          <div class="triage-candidate-left">
            <img class="triage-avatar" src="${cand.avatar_url}" alt="${escapeHtml(cand.display_name)}" data-key="${cand.key}" onerror="this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(cand.handle)}&background=1e293b&color=fff';" title="Click to inspect avatar &amp; reverse image search" />
  ...
  elBtnCopyHarvestedIntel?.addEventListener('click', () => {
    ...
    showToast('Copied Harvested Intelligence Pool JSON to clipboard!');
  });
      ...
      return `
        <div class="social-profile-card ${isConfirmed || isVerified ? 'verified-found' : ''}" data-url="${p.url}">
          <div class="social-card-top">
            <div class="social-avatar-wrapper" title="Click to view/compare avatar" data-url="${avatarSrc}">
  ...
  function openAvatarCompare(candidate) {
    ...
    elModalAvatarCompare.classList.add('active');
  }

  elBtnCloseAvatarCompare?.addEventListener('click', () => elModalAvatarCompare?.classList.remove('active'));
  ...
  function openCryptoDrawer() {
    ...
    let html = `
      <div style="background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.08);border-radius:6px;padding:12px;">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;font-size:0.68rem;color:var(--text-muted);">
          ...
          <div style="display:flex;gap:6px;">
            <button type="button" class="btn-icon" style="padding:2px 8px;font-size:0.66rem;" onclick="navigator.clipboard.writeText('${pgpHex}'); showToast('Copied PGP Fingerprint to clipboard!');">📋 Copy Hex</button>
    ...
    elModalCryptoDrawer.classList.add('active');
  }

  elBtnOpenCryptoDrawer?.addEventListener('click', () => openCryptoDrawer());
  elBtnCloseCryptoDrawer?.addEventListener('click', () => elModalCryptoDrawer?.classList.remove('active'));
  ...
  function openEmailMatrixDrawer() {
    ...
    let html = `
      <div style="background:rgba(0,0,0,0.3);border:1px solid rgba(255,255,255,0.08);border-radius:6px;padding:12px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          ...
          <button type="button" class="btn-icon" style="padding:2px 8px;font-size:0.66rem;" onclick="navigator.clipboard.writeText('${perms.join('\\n')}'); showToast('Copied all email permutations to clipboard!');">📋 Copy All</button>
        </div>
      ...
      html += `
        <div class="email-perm-row">
          ...
          <div style="display:flex;gap:4px;">
            <button type="button" class="btn-icon" style="padding:1px 5px;font-size:0.60rem;" onclick="navigator.clipboard.writeText('${em}'); showToast('Copied: ${em}');">📋 Copy</button>
    ...
    elModalEmailMatrix.classList.add('active');
  }

  elBtnOpenEmailDrawer?.addEventListener('click', () => openEmailMatrixDrawer());
  elBtnCloseEmailMatrix?.addEventListener('click', () => elModalEmailMatrix?.classList.remove('active'));
  ...
  function openChronoDrawer() {
    ...
    elModalChronoDrawer.classList.add('active');
  }

  elBtnOpenChronoDrawer?.addEventListener('click', () => openChronoDrawer());
  elBtnCloseChronoDrawer?.addEventListener('click', () => elModalChronoDrawer?.classList.remove('active'));
  ...
  document.getElementById('btn-social-consolidate-person')?.addEventListener('click', () => {
    ...
    const u = elSocialInput.value.trim() || harvestedContext.primaryQuery || 'target_user';
    showToast(`Copied Forensic Persona Dossier for "${u}" to clipboard!`);
  });
  ...
  window.addNodeToGraph = function(label, type = 'ip', customX = null, customY = null) {
    ...
    const newNode = {
      ...
      label: label.trim(),
      type: type,
      x: customX !== null ? customX : (cx + (Math.random() * 100 - 50)),
      y: customY !== null ? customY : (cy + (Math.random() * 100 - 50)),
      vx: 0,
      vy: 0,
  ...
  const tourSlides = [
    {
      ...
      desc: 'Welcome to your tactical OSINT & intelligence start page. Loaded with <strong>1,699+ verified tools</strong>, Australian corporate & legal registries, Google AI Studio, and sub-millisecond search.',
